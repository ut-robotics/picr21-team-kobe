...
def handle_aim(state_data, gamepad):
    ...
    print(state_data.basket_x) 
                                #415                                                    433                                             410
    if basket_in_frame and camera.camera_x * 0.489 <= state_data.basket_x <= camera.camera_x * 0.51 and state_data.ball_y >= camera.camera_y * 0.855: # Start throwing if ball y is close to robot and basket is centered to camera x
        #print("here")
...
def handle_throwing(state_data, gamepad):
    ...
    if state_data.keypoint_count >= 1 and not state_data.has_thrown:
        ...
        side_speed = calc_speed(delta_x, camera.camera_x, 5, 2, 150, max_speed)
        rot_spd = calc_speed(rot_delta_x, camera.camera_x, 5, 1, 100, max_speed)
        state_data.prev_yspeed = front_speed
        state_data.prev_rotspeed = rot_spd
        state_data.prev_xspeed = side_speed
